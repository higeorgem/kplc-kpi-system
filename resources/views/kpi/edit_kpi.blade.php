    ...
    <div class="">
        ...
        <div class="card row justify-content-center">
            <div class=" shadow col-md-">
                <div class="card-header text-uppercase h4 text-center bg-dark">Edit {{$kpi->kpi}} KPI</div>
                <div id="validation-errors"></div>
                <form action="{{route('kpi.update', [$kpi->id])}}" method="post" id="kpi_form" class="card-body">
                    @method('PUT')
                    @csrf
                    <div class="form-group row">
                        <div class="col-md-6">
                            <label for="perspective">Perspective: </label>
                            <select id="perspective" class="form-control @error('perspective') is-invalid @enderror" name="perspective">
                                ...
                                <option value="{{$perspective->id}}" @if (old('perspective',$kpi->perspective) == $perspective->id) selected="selected"  @endif>
                                    {{ucfirst($perspective->perspective)}}</option>
                                @empty
                        ...
                        <div class="col-md-6">
                            ...
                            <select id="kpi_type" class="form-control @error('kpi_type') is-invalid @enderror"
                                ...
                                <option value="Tasked" @if (old('kpi_type', $kpi->kpi_type)=="Tasked" )
                                    selected="selected" @endif>
                                    Tasked</option>
                                <option value="NotTasked" @if(old('kpi_type', $kpi->kpi_type)=="NotTasked" )
                                    selected="selected" @endif>
                                    Not Tasked</option>
                            </select>
                            @error('kpi_type')
                    ...
                    <div class="form-group row">
                        <div class="col-sm-6">
                            ...
                            <select id="structure" class="form-control @error('structure') is-invalid @enderror"
                                ...
                                <option value="divisions" @if (old('structure', $kpi->structure)== "divisions" )
                                    selected="selected" @endif >Division</option>
                                <option value="departments" @if (old('structure', $kpi->structure)== "departments" )
                                    selected="selected" @endif>Department</option>
                                <option value="sections" @if (old('structure', $kpi->structure)== "sections" )
                                    selected="selected" @endif>Section</option>
                                <option value="sub_sections" @if (old('structure', $kpi->structure) == "sub_sections")
                                    selected="selected" @endif>Sub-Section</option>
                            </select>
                            @error('structure')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                        ...
                        <div class="col-sm-6">
                            <label for="structure_id" id="structure_label">Select Structure Type:</label>
                            <select id="structure_id"
                                class="form-control @error('structure_id') is-invalid @enderror structure_id"
                                name="structure_id">
                                <option value="" selected disabled>Select Structure Type:</option>
                                <div id="cont_structure_id">
                                    @forelse ($structures as $structure)
                                    <option id="default_id" value="{{$structure->id}}" @if (old('structure_id', $kpi->structure_id) == $structure->id) selected="selected" @endif >{{$structure->name}}</option>
                                    @empty
                    ...
                    <div class="form-row">
                        ...
                        <input id="period" type="text" class="form-control @error('period') is-invalid @enderror"
                            name="period" value="{{ old('period',$kpi->period) }}" autocomplete="period"
                            placeholder="20-21">
                        @error('period')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
            ...
            {{-- {{$kpi->unitofmeasure($kpi->kpiunitofmeasure->first()->unit_of_measure)->id }} --}}
            <div class="form-row">
                <div class="form-groupv col-sm-6">
                    ...
                    <select name="unit_of_mesure" id="unit_of_mesure"
                        ...
                        <option value="{{$uom->id}}"
                            @php
                                $test = $kpi->unitofmeasure($kpi->kpiunitofmeasure->first()->unit_of_measure)
;                            @endphp
                             @if ($test && $test->id == $uom->id)
                                selected
                            @endif > {{ucfirst($uom->unit_of_measure)}}</option>
                        @empty
                        <option value="" selected disabled>No Units Of Measure</option>
                        @endforelse
                    </select>
                ...
                <div class="form-group col-sm-6">
                    ...
                    <input id="weight" type="text" class="form-control @error('weight') is-invalid @enderror"
                        name="weight" value="{{ old('weight', (($kpi->kpiWeight->first())== null) ? 0 : $kpi->kpiWeight->first()->weight) }}"
                        autocomplete="weight">
                    @error('weight')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
            ...
            <div class="form-row">
                <div class="form-group col-sm-6">
                    ...
                    <input id="target" type="text" class="form-control @error('target') is-invalid @enderror"
                        name="target" value="{{ old('target', (count($kpi->kpiTarget) == 0 ? 0 : $kpi->kpiTarget->first()->target)) }}"
                        autocomplete="target">
                    @error('target')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
            ...
            {{-- {{dd(count($kpi->kpiTarget))}} --}}
                <div class="form-group col-sm-6">
                    ...
                    <input id="previous_target" type="text"
                        ...
                        value="{{ old('previous_target', (count($kpi->kpiTarget) <= 1 ? 0 : $kpi->getPreviousTargets($kpi->id)[1]->target)) }}" disabled>
                ...
                {{-- {{dd($kpi->kpiAchievement->first()->validated_achievement)}} --}}
                <div class="form-group col-sm-6">
                    ...
                    <input id="validated_achievement" type="text"
                        ...
                        name="validated_achievement"
                        value="{{ old('validated_achievement', (count($kpi->kpiAchievement) == 0 ? 0 : $kpi->kpiAchievement->first()->validated_achievement)) }}"
                ...
                <div class="form-group col-sm-6">
                    ...
                    <input id="achievement" type="text" class="form-control @error('achievement') is-invalid @enderror"
                        name="achievement"
                        value="{{ old('achievement',(count($kpi->kpiAchievement) == 0 ? 0 : $kpi->kpiAchievement->first()->id)) }}"
            ...
            <div class="card-footer text-center">
                <button type="submit" id="form_submit" class="btn btn-outline-success">
                    <i class="icofont icofont-refresh"></i> Update KPI
                </button>
            </div>
            </form>
        </div>
